...
import { controlMatch, correctResult, deleteAnnouncement, deletePlayer, deleteTeam, getSeasonData, saveAnnouncement, saveMatch, savePlayer, saveSeason, saveTeam } from '../../services/hockeyService';
...
export default function AdminAnjk3() {
  ...
  const matchAction = async (action, teamId, playerId) => {
    const saved = await run(() => controlMatch(selectedMatch.id, action, teamId, playerId || null), 'Match updated.');
  ...
  return <Box sx={{ maxWidth: 1100, mx: 'auto', pb: 8 }}>
    ...
    {tab === 'Fixtures' && <Stack spacing={2}><Paper sx={{ p: 2 }}><Typography variant="h6" sx={{ mb: 2 }}>{matchForm.id ? 'Edit / reschedule fixture' : 'Create fixture'}</Typography><Grid container spacing={2}>
      ...
      <Grid item xs={12} sm={6}>{field('Date and time',matchForm.scheduled_at,(v) => setMatchForm({ ...matchForm, scheduled_at:v }),{ type:'datetime-local', InputLabelProps:{ shrink:true } })}</Grid>
    ...
    {tab === 'Match Control' && <HockeyMatchControl matches={data?.matches || []} players={data?.players || []} selectedMatch={selectedMatch} selectMatch={selectMatch} scorers={scorers} setScorers={setScorers} busy={busy} matchAction={matchAction} correct={correct} setCorrect={setCorrect} saveCorrection={() => run(() => correctResult(selectedMatch.id, correct.home, correct.away), 'Result corrected; standings updated.')} confirmEnd={confirmEnd} setConfirmEnd={setConfirmEnd} />}
