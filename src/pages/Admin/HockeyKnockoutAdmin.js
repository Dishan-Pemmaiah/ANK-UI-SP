  ...
  return <Stack spacing={2}>
    ...
    {ordered.map((round) => {
      ...
      return <Paper key={round.id} sx={{p:{xs:1.5,sm:2},minWidth:0}}>
        ...
        {round.is_final && fixtures.filter((match) => match.status==='Completed' && match.winner_team_id && season.champion_team_id!==match.winner_team_id).map((match) => <Button key={match.id} sx={{mt:1}} onClick={() => onSetChampion(match.winner_team_id)}>Set {teams.find((team) => team.id===match.winner_team_id)?.name} as champion</Button>)}
      </Paper>;
    })}
