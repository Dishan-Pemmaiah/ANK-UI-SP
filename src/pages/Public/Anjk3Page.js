...
import HockeyDrawTies from './HockeyDrawTies';
...
const allTabs = ['Overview', 'Draw / Ties', 'Fixtures', 'Live', 'Results', 'Points Table', 'Knockouts', 'Teams', 'Stats'];
const slug = (tab) => tab === 'Draw / Ties' ? 'draw' : tab.toLowerCase().replace(' ', '-');
...
export default function Anjk3Page() {
  ...
  const tabs = data?.season?.tournament_format === 'Knockout' ? allTabs.filter((item) => item !== 'Points Table') : allTabs;
  ...
  const currentDraw = (data?.drawDocuments || []).find((document) => document.is_published && document.is_current)
    || (data?.drawDocuments || []).filter((document) => document.is_published).sort((a,b) => b.version_number-a.version_number)[0];
  ...
  return <Box sx={{ maxWidth: 1200, mx: 'auto', pb: 6 }}>
    ...
    {tab === 'Overview' && <Stack spacing={3}>
      {currentDraw && <Paper sx={{ p:2,border:'1px solid #694334' }}><Typography variant="overline" sx={{color:'#e0b08f'}}>Official Draw / Ties</Typography><Typography variant="h6" fontWeight={800}>{currentDraw.title}</Typography><Typography variant="body2" color="text.secondary">Version {currentDraw.version_number} · Updated: {new Date(currentDraw.updated_at).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short',timeZone:'Asia/Kolkata'})} IST</Typography>{currentDraw.revision_note && <Alert severity="info" sx={{mt:1}}>Updated draw: {currentDraw.revision_note}</Alert>}<Button component={Link} to={`${basePath}?tab=draw`} variant="outlined" sx={{mt:1,minHeight:44}}>View Draw</Button></Paper>}
    ...
    {tab === 'Draw / Ties' && <HockeyDrawTies documents={data?.drawDocuments || []} />}
    {['Fixtures','Results'].includes(tab) && <Stack spacing={2}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ sm: 'center' }}>
        ...
        <TextField select label="Stage" value={stage} onChange={(e) => setStage(e.target.value)} size="small" sx={{ minWidth: 150 }}><MenuItem value="">All stages</MenuItem>{[...new Set((data?.matches || []).map((match) => match.stage))].map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}</TextField>
    ...
    {tab === 'Knockouts' && <HockeyKnockoutBracket rounds={data?.rounds || []} matches={(data?.matches || []).filter((match) => data?.season?.tournament_format === 'Knockout' || match.round_id || (data?.season?.tournament_format === 'Mixed' && !['Group','League'].includes(match.stage)))} teams={data?.teams || []} championId={season?.champion_team_id} basePath={basePath} />}
