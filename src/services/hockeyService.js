...
export const getSeasonData = async (slug = 'anjk-3') => {
  ...
  const [teams, matches, standings, announcements, players, events] = await Promise.all([
    ...
    table('hockey_announcements').select('*').eq('season_id', id).order('created_at', { ascending: false }),
...
export const controlMatch = async (matchId, action, teamId = null) => unwrap(await supabase.rpc('hockey_control_match', {
  p_match_id: matchId, p_action: action, p_team_id: teamId
}));
