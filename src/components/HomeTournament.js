...
export default function HomeTournament() {
  ...
  return <Paper sx={{ p: { xs: 2, md: 3 }, mb: 4, borderRadius: 4, background: 'linear-gradient(135deg,#231010,#141414)', border: '1px solid #5a2525' }}>
    ...
    {(state.beforeStart || (season.starts_at && new Date(season.starts_at) > new Date() && !state.results.length)) && <Stack spacing={1} sx={{ mb: 2 }}>{season.poster_url && <Box component="img" src={season.poster_url} alt={`${season.name} poster`} sx={{ width:'100%', maxHeight:260, objectFit:'cover', borderRadius:2 }} />}{days !== null && <Typography fontWeight={800}>{days === 0 ? 'Tournament starts today' : `${days} days until the tournament`}</Typography>}</Stack>}
